...
short-title: "Derogations and Democratic Backsliding"
author:
...
code-repo: Access the code and data at <https://github.com/andrewheiss/covid-derogations>.
abstract: >-
...
keywords: "international human rights treaties, civil society, COVID-19, democracy"
...
tbl_pandem_responses <- pandem_single %>% 
...
if (!is_docx) {
  ...
  tbl_pandem_responses %>% 
    ...
    huxtable::set_caption("National emergency responses to the COVID-19 pandemic") %>% 
...
tbl_derog_typology <- pandem_derog %>% 
...
if (!is_docx) {
  ...
  tbl_derog_typology %>% 
    ...
    set_bold(1, everywhere) %>%
    set_bottom_border(1, everywhere) %>% 
    huxtable::set_caption("Typology of possible pandemic responses across derogations and emergency responses") %>% 
    huxtable::as_flextable()
}
```
...
tbl_derog_a_b <- tbl_derog %>% 
  select(Country = country_name, `Derogation start` = derogation_start, 
         `Derogation end` = derogation_end) 

tbl_a_b_breaks <- table(fct_inorder(fct_drop(tbl_derog$type_clean_letter)))
tbl_a_b_caption <- "Countries that either derogated as intended or derogated with excessive human rights violations"

if (!is_docx) {
  tbl_derog_a_b %>% 
    kbl(caption = tbl_a_b_caption,
        booktabs = TRUE, linesep = "") %>% 
    kable_styling() %>% 
    pack_rows(index = tbl_a_b_breaks)
} else {
  tbl_derog_a_b %>% 
    hux() %>% 
    set_font_size(10) %>%
    set_all_padding(3) %>% 
    insert_row(names(tbl_a_b_breaks[1]), "", "", after = 1) %>%
    insert_row(names(tbl_a_b_breaks[2]), "", "", 
               after = (tbl_a_b_breaks[1] + 2)) %>%
    set_bold(1, everywhere) %>%
    set_bold(2, everywhere) %>%
    set_bold((tbl_a_b_breaks[1] + 3), everywhere) %>% 
    set_bottom_border(1, everywhere) %>%
    set_bottom_border(2, everywhere) %>%
    set_bottom_border((tbl_a_b_breaks[1] + 3), everywhere) %>%
    huxtable::set_caption(tbl_a_b_caption) %>% 
    huxtable::as_flextable()
}
```
...
tbl_case_summary_caption <- "Summary of case studies"

tbl_case_notes <- c("Pandemic violations index ranges from 0 to 0.65; higher values represent worse violations", 
...
if (!is_docx) {
  tbl_case_summary %>% 
    kbl(align = c("l", "l", "c", "c", "c", "l"),
        caption = tbl_case_summary_caption,
    ...
    footnote(general = tbl_case_notes)
} else {
  tbl_case_summary %>% 
    hux() %>% 
    set_font_size(10) %>%
    ...
    set_align(everywhere, c(1, 2, 6), "left") %>% 
    set_bold(1, everywhere) %>%
    set_bottom_border(1, everywhere) %>%
    huxtable::add_footnote(paste(tbl_case_notes, collapse = " • ")) %>% 
    huxtable::set_caption(tbl_case_summary_caption) %>% 
    huxtable::as_flextable()
}
```
...
tbl_derog_c_d <- matrix_combined[,-1]
tbl_c_d_breaks <- table(matrix_combined[,1])
tbl_c_d_caption <- "Countries that did not derogate during the pandemic"

if (!is_docx) {
  tbl_derog_c_d %>% 
    kbl(caption = tbl_c_d_caption,
        booktabs = TRUE, linesep = "") %>% 
    kable_styling(latex_options = c("hold_position")) %>% 
    pack_rows(index = tbl_c_d_breaks, indent = FALSE)
} else {
  tbl_derog_c_d %>% 
    hux() %>% 
    set_font_size(10) %>%
    set_all_padding(3) %>% 
    insert_row(names(tbl_c_d_breaks[1]), "", "", "", after = 1) %>%
    insert_row(names(tbl_c_d_breaks[2]), "", "", "",
               after = (tbl_c_d_breaks[1] + 2)) %>%
    set_bold(1, everywhere) %>%
    set_bold(2, everywhere) %>%
    set_bold((tbl_c_d_breaks[1] + 3), everywhere) %>% 
    set_bottom_border(1, everywhere) %>%
    set_bottom_border(2, everywhere) %>%
    set_bottom_border((tbl_c_d_breaks[1] + 3), everywhere) %>%
    huxtable::set_caption(tbl_c_d_caption) %>% 
    huxtable::as_flextable()
}
```
